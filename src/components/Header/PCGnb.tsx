...
function PCGnb({ openMenuIndex, setOpenMenuIndex }: PCGnbProps) {
  const [user, setUser] = useState<any>(null); // ✅ 로그인 상태 저장
    ...
    <nav className="hidden md:flex items-center space-x-6">
      ...
      {user ? (
        <button
          ...
          className="ml-4 px-4 py-[5px] bg-red-600 text-white font-semibold rounded hover:bg-red-700 transition"
        >
          LOGOUT
        </button>
      ) : (
        <Link
          href="/login"
          className="ml-4 px-4 py-[5px] bg-white text-green-900 font-semibold rounded hover:bg-gray-200 transition"
        >
          LOGIN
        </Link>
      )}
